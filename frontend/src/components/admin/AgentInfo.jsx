...
const AgentInfo = () => {
   ...
   const handleSubmit = async (userId) => {
      if (updateAgent === "") {
         ...
         axios.put(`http://localhost:8080/admin/update/${userId}`, updateAgent)
            .then((res) => {
                  ...
                  <tbody>
                     {ordinaryList.length > 0 ? (
                        ordinaryList.map((agent) => {
                           const open = toggle[agent.userId] || false;
                           ...
                           return (
                              <tr key={agent.userId}>
                                 ...
                                 <td>{agent.email}</td>
                                 <td>{agent.phone}</td>
                                 <td><Button onClick={() => handleToggle(agent.userId)}
                                    aria-controls={`collapse-${agent.userId}`}
                                    aria-expanded={open}
                                    className='mx-2'
                                    ...
                                    <Collapse in={open}>
                                       <Form onSubmit={() => handleSubmit(agent.userId)} className='p-5'>
                                    ...
                                    <Button onClick={() => deleteUser(agent.userId)} className='mx-2' variant="outline-danger">Delete</Button></td>
                              </tr>
                           )
                        })
                     ) : (
